...
![version](https://img.shields.io/badge/版本-v2.7-4caf50?style=flat-square)
...
| **觸發記錄** | 顯示最近 150 筆事件，含排程觸發、心跳、SW 喚醒、視窗狀態 |
| **清除** | 清空觸發記錄 |

> 自動排程若未如期執行，先展開診斷面板查看是否有 `alarm_fired`、`heartbeat`、`sw_boot` 紀錄。若完全沒有紀錄，多半是「背景執行」設定未開啟。
