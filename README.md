...
> `setup.bat` 同時會登錄更新通訊協定，執行後才能使用擴充套件設定頁中的「立即更新」功能。
...
> 需先執行過 `setup.bat`，設定頁才會顯示更新控制項。若移動過安裝資料夾，需重新執行 `setup.bat`。
...
<details>
<summary>設定頁看不到「檢查更新」？</summary>
<br>

請先執行 `setup.bat`，完成後在設定頁點擊「我已執行 setup.bat」，更新功能即會解鎖。
...
通訊協定可能因資料夾移動而失效，請重新執行 `setup.bat` 再試。
